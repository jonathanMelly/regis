...
func TestBuildManifest_scenarios(t *testing.T) {
	m := runner.BuildManifest("v1", []string{"app", "db"}, nil, nil, "")
...
func TestWriteManifest_uploadsToCorrectPath(t *testing.T) {
	up := &mockUploader{}
	m := runner.BuildManifest("v20260101-120000", []string{"app"}, nil, nil, "")
...
func TestSnapshotRelease_emptyIDorDir_isNoop(t *testing.T) {
	dir := t.TempDir()
	manifest := runner.BuildManifest("v1", nil, nil, nil, "")
...
func TestBuildManifest_cueArtifacts_fromPackResult(t *testing.T) {
	...
	m := runner.BuildManifest("v20260607-120000", []string{"App"}, results, nil, "/opt")
...
func TestBuildManifest_cueArtifacts_fromBinaryStep(t *testing.T) {
	...
	m := runner.BuildManifest("v1", []string{"App"}, nil, steps, "/opt/app")
...
func TestSnapshotRelease_packUsesLocalArtifacts(t *testing.T) {
	...
	manifest := runner.BuildManifest("v20260607-120000", []string{"App"}, results, steps, "/var/www")
